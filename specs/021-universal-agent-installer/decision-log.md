...
| DEC-004 | 2026-08-05 | accepted | Harness Maintainers | Make update record-driven and refuse local managed drift | Requiring users to restate platform profile/root/modules is error-prone, while unconditional replacement can erase reviewed local changes | Keep the manual update recipe; accept update selectors; recover only verified provenance | install.py; install.sh; native installer; update guide; tests | FR-010; AC-009; TC-008 |
