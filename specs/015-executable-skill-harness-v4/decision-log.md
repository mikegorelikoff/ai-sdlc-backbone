...
| DEC-009 | 2026-07-30 | accepted | Release Owner and Harness Maintainers | Preserve immutable v4.0.0 and publish corrective v4.0.1 with a native TOON-only installer | Fresh tagged installation proved the external installer generated a non-TOON lock and ignored the requested structured-list flag; user requires TOON-only machine boundaries | Rewrite consumed v4.0.0 tag; retain the incompatible external path; publish a new immutable patch | install.sh; native installer; install schemas; install/update docs; release-4.0.md | DEC-007; AC-010; AC-013; post-publication smoke |
