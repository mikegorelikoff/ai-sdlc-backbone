...
class MetricsTests(unittest.TestCase):
    ...
    def test_empty_metrics_are_explicit_and_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repository = Path(temp); first = self.cli(repository, "--format", "toon"); second = self.cli(repository, "--format", "toon")
            self.assertEqual(first.stdout, second.stdout); self.assertEqual(toon_codec.loads(first.stdout)["status"], "insufficient-data")
    def test_aggregates_runs_and_evidence_without_content(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            state = {"schema":"ai-sdlc-run-state/v2","status":"completed","budgets":{"used_steps":2,"used_failures":1,"used_tokens":55},"tasks":[{"status":"succeeded","attempts":2},{"status":"succeeded","attempts":1}],"fingerprint":"a"*64,"secret_payload":"DO-NOT-COLLECT"}
            (run / "state.toon").write_text(toon_codec.dumps(state), encoding="utf-8")
            ...
            ledger_path = repository / "_ai_sdlc/evidence-ledger.toon"; ledger_path.parent.mkdir(exist_ok=True); ledger_path.write_text(toon_codec.dumps(ledger), encoding="utf-8")
            result = self.cli(repository, "--write", "--format", "toon"); self.assertEqual(result.returncode, 0, result.stdout + result.stderr); value = toon_codec.loads(result.stdout)
    ...
    def test_write_rejects_symlinked_repository_parent(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = self.cli(repository, "--write", "--format", "toon")
            self.assertEqual(result.returncode, 1)
            self.assertIn("symlink", result.stdout)
            self.assertFalse((outside / "metrics/local.toon").exists())
