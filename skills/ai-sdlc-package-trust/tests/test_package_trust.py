...
class PackageTrustTests(unittest.TestCase):
    ...
    def test_valid_package_is_deterministic_and_writes_toon(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            first = self.cli(repository, package, manifest, "--write", "--format", "toon"); second = self.cli(repository, package, manifest, "--format", "toon")
            self.assertEqual(first.returncode, 0, first.stdout + first.stderr); self.assertEqual(first.stdout, second.stdout); self.assertEqual(toon_codec.loads(first.stdout)["decision"], "allow")
    ...
    def test_tamper_disallowed_capability_and_missing_provenance_deny(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = self.cli(repository, package, manifest, "--format", "toon"); self.assertEqual(result.returncode, 2); value = toon_codec.loads(result.stdout); self.assertEqual(value["decision"], "deny")
    ...
    def test_symlink_inventory_fails_integrity(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = self.cli(repository, package, manifest, "--format", "toon"); self.assertEqual(result.returncode, 2); self.assertIn("integrity-fail", toon_codec.loads(result.stdout)["reason_codes"])
