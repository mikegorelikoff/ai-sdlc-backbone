...
class PolicyTests(unittest.TestCase):
    ...
    def context(self, repository: Path, subject: str = "change:add-audit", **values: object) -> Path:
        path = repository / "context.toon"
        path.write_text(toon_codec.dumps({"subject": subject, **values}), encoding="utf-8")
        return path
    ...
    def layer(self, repository: Path, scope: str, rules: list[dict[str, object]], name: str | None = None) -> Path:
        path = repository / f"{name or scope}.toon"
        path.write_text(toon_codec.dumps({"schema": "ai-sdlc-policy-layer/v1", "id": name or scope, "version": "1.0.0", "scope": scope, "rules": rules}), encoding="utf-8")
        return path

    def waiver(self, repository: Path, rule_id: str, expires_at: str, action: str = "deploy.preview", subject: str = "change:add-audit") -> Path:
        path = repository / f"{rule_id}-waiver.toon"
        path.write_text(toon_codec.dumps({"schema": "ai-sdlc-policy-waiver/v1", "id": f"waive-{rule_id}", "rule_id": rule_id, "actions": [action], "subject": subject, "constraints": {"risk": "accepted"}, "owner": "Delivery", "approved_by": "Security", "decision_ref": "DEC-100", "reason": "Bounded migration window.", "issued_at": "2026-07-18T00:00:00Z", "expires_at": expires_at}), encoding="utf-8")
        return path

    def test_profile_resolution_is_deterministic_with_provenance(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repository = Path(temp)
            first = self.cli(repository, "--resolve", "--profile", "high-assurance", "--write", "--format", "toon")
            second = self.cli(repository, "--resolve", "--profile", "high-assurance", "--format", "toon")
            self.assertEqual(first.returncode, 0, first.stdout + first.stderr)
            self.assertEqual(first.stdout, second.stdout)
            resolution = toon_codec.loads(first.stdout)
    ...
    def test_high_assurance_apply_requires_union_of_gates(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = self.cli(repository, "--evaluate", "change.apply", "--context", str(context), "--profile", "high-assurance", "--as-of", "2026-07-19T00:00:00Z", "--format", "toon")
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            decision = toon_codec.loads(result.stdout)
    ...
    def test_deny_precedence_and_unknown_action_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repository = Path(temp)
            context = self.context(repository)
            denied = toon_codec.loads(self.cli(repository, "--explain", "command.destructive", "--context", str(context), "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
            self.assertEqual(denied["result"], "deny")
            unknown = toon_codec.loads(self.cli(repository, "--evaluate", "future.unknown", "--context", str(context), "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
    ...
    def test_current_waiver_applies_but_expired_and_nonwaivable_do_not(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            allowed = toon_codec.loads(self.cli(repository, "--evaluate", "deploy.preview", "--context", str(context), "--organization", str(organization), "--waiver", str(current), "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
            ...
            required = toon_codec.loads(self.cli(repository, "--evaluate", "deploy.preview", "--context", str(context), "--organization", str(organization), "--waiver", str(expired), "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
            ...
            rejected = toon_codec.loads(self.cli(repository, "--evaluate", "change.apply", "--context", str(context), "--waiver", str(base_waiver), "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
    ...
    def test_regulated_profile_adds_controls_and_conditional_deny(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            data = toon_codec.loads(self.cli(repository, "--evaluate", "data.production.read", "--context", str(context), "--profile", "regulated", "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
            self.assertEqual(data["result"], "deny")
            release = toon_codec.loads(self.cli(repository, "--evaluate", "release.publish", "--context", str(context), "--profile", "regulated", "--as-of", "2026-07-19T00:00:00Z", "--format", "toon").stdout)
