...
class DoctorTests(unittest.TestCase):
    ...
    def test_repository_doctor_is_deterministic_and_healthy(self) -> None:
        first = self.cli(ROOT, "--doctor", "--format", "toon")
        second = self.cli(ROOT, "--doctor", "--format", "toon")
    ...
    def test_broken_layout_has_actionable_failures_without_repair(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = self.cli(repository, "--doctor", "--format", "toon")
    ...
    def test_upgrade_plans_changes_migration_backup_and_reverse_rollback(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repository = Path(temp)
            args = ("--upgrade", "--current", str(FIXTURES / "current.toon"), "--target", str(FIXTURES / "target.toon"), "--upgrade-id", "upgrade-110", "--write", "--format", "toon")
            ...
            self.assertEqual({item["path"] for item in value["backups"]}, {"config/policy.toon", "skills/legacy/SKILL.md"})
    ...
    def test_incompatible_api_and_unsafe_inventory_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            repository = Path(temp)
            result = self.cli(repository, "--upgrade", "--current", str(FIXTURES / "current.toon"), "--target", str(FIXTURES / "incompatible.toon"), "--upgrade-id", "upgrade-200", "--format", "toon")
            self.assertEqual(result.returncode, 2)
            self.assertIn("target-harness-api-incompatible", toon_codec.loads(result.stdout)["blockers"])
            unsafe = toon_codec.loads((FIXTURES / "target.toon").read_text(encoding="utf-8"))
            unsafe["files"][0]["path"] = "../escape"
            path = repository / "unsafe.toon"
            path.write_text(toon_codec.dumps(unsafe), encoding="utf-8")
            result = self.cli(repository, "--upgrade", "--current", str(FIXTURES / "current.toon"), "--target", str(path), "--upgrade-id", "unsafe")
    ...
    def test_write_rejects_symlinked_repository_parent(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertFalse((outside / "doctor/report.toon").exists())
