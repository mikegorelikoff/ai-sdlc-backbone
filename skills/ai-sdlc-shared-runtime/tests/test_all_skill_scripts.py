...
def script_paths() -> list[Path]:
    ...
    paths.extend(sorted((ROOT / "skills" / "ai-sdlc-shared-runtime" / "tests").glob("*.py")))
...
class ScriptContractTests(unittest.TestCase):
    ...
    def test_install_smoke_absolutizes_local_source_before_consumer_run(self) -> None:
        ...
        path = ROOT / "skills/ai-sdlc-shared-runtime/tests/install_smoke.py"
    ...
    def test_context_benchmark_reports_positive_savings_without_a_percentage_gate(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=ROOT) as temp_dir:
            ...
            result = run_script(
                ROOT / "skills/ai-sdlc-shared-runtime/scripts/ai_sdlc_context_benchmark.py",
    ...
    def test_specs_index_cli_writes_toon_and_markdown_indexes(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=ROOT) as temp_dir:
            ...
            result = run_script(
                ROOT / "skills/ai-sdlc-shared-runtime/scripts/ai_sdlc_specs_index.py",
                "--workspace",
                "refinement",
                "--full-flow",
                cwd=cwd,
        ...
        for skill_doc in sorted((ROOT / "skills").glob("*/SKILL.md")):
            with self.subTest(skill_doc=skill_doc.relative_to(ROOT)):
                ...
                self.assertNotIn(
                    "skills/_shared",
                    text,
                    "skills must resolve only the canonical installable runtime",
                )
