...
SCRIPT = ROOT / "skills" / "ai-sdlc-shared-runtime" / "scripts" / "ai_sdlc_handoff.py"


def contract_text(skill: Path) -> str:
    text = skill.read_text(encoding="utf-8")
    manifest = json.loads(
        (skill.parent / "steps" / "manifest.json").read_text(encoding="utf-8")
    )
    return text + "\n".join(
        (skill.parent / selector["path"]).read_text(encoding="utf-8")
