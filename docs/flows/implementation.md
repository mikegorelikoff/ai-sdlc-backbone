...
description: Move from accepted delivery evidence through task branching, SDD, bounded tasks, repository-grounded engineering quality review, validation, review, and traceable commits.
---
...
| `task_context` | Accepted `sdd` task or ready semantic StepCard, or evidence-backed existing contract for direct review. | Dev | Task/step/AC/TC IDs, canonical step document, mandatory anchors, repository topology, ownership/test evidence, exclusions, and token budget. | `ai-sdlc-project-context` plus the shared step-context compiler | Fresh project context and canonical `ai-sdlc-context-pack/v4`, with optional human context review. | Selected and skipped sources are explained; instruction authority is explicit; critical recall is complete; fingerprint is current; budget and savings pass, or exact direct reads are required. | `implement`, `validate`, or `review` according to the requested task. | Source drift, stale pack, missing critical anchor, insufficient savings, missing ownership/test topology, or unsafe selected path. |
| `implement` | `sdd` + `task_context` | Dev | One ready task, linked AC/TC, accepted design, bounded context. | Host coding agent within declared task scope | Code, tests, and bounded working-tree diff linked to the task. | Tests are derived first; requested behavior is present; task remains open until trusted validation passes. | `quality_gate` when selected by the owning workflow or policy; otherwise `validate`. | Scope expansion, changed acceptance, implementation blocker, or failing trusted regression. |
| `quality_gate` | `implement`, or direct post-implementation request with an authoritative contract and bounded diff. It is mandatory in AI SDLC Loop and whenever the owning workflow selects it. | Dev with QA/Architecture/Security support as relevant | Requested change, current diff, repository instructions, comparable local implementations, and repository-owned verification sources. | `ai-sdlc-engineering-quality-gate` | Canonical context and evidence-based quality report with typed findings, safe material fixes, exact check outcomes, current fingerprints, and readiness decision. | No unresolved High or blocking Medium finding remains; required available checks pass; the report is current and `ready_for_next_stage` is true. | `validate`, or the exact owner of a finding, clarification, authority, scope, or failed-check blocker. | Diff/context drift, new material finding, failed or unexecuted required available check, unsafe remediation, or changed requirement. |
| `validate` | `implement`, current selected `quality_gate` evidence, or a direct validation-only request with an authoritative contract. | Dev/QA | Current diff/target, AC/TC, QA scope, exact environment constraints. | `ai-sdlc-validation` | Exact command outcomes, coverage mapping, skipped checks, and residual-risk handoff. | Required focused checks pass and evidence is current for the same diff. | `review`, or `implement` when a failure requires repair. | Source changes after validation, failed/skipped required check, or stale environment evidence. |
...
| `release_handoff` | Accepted `commit_prep` evidence and release candidate. | Release/Delivery | Commit/build identity, rollout, monitor, rollback, owners, waivers, residual risk. | `ai-sdlc-validation` plus accountable release handoff | Accepted release evidence and `ai-sdlc-handoff/v2` to the release owner. | Required release checks, owners, rollout/monitor/rollback, and residual risk are accepted. | Release execution and later `ai-sdlc-retrospective`. | Stale build/evidence, expired waiver, unowned dependency, failed rollout, or rollback trigger. |
