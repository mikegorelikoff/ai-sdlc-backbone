---
title: Project
description: Review AI SDLC Harness status, limitations, security, privacy, compatibility, releases, audits, and maintainer guidance.
---

# Project

Use this section to assess adoption risk, report a problem, contribute, or
understand why the documentation and product are structured this way.

## Status and boundaries

- [Maturity and limitations](../explanation/maturity-limitations.md)
- [Compatibility contract](../reference/compatibility.md)
- [Supported environments](../reference/supported-environments.md)
- [Roadmap](../roadmap.md)

## Security, privacy, and trust

- [Security and privacy](security-privacy.md)
- [Governance and trust](../operations/governance.md)
- [Validation contracts](../reference/validation.md)
- [Report a vulnerability](https://github.com/mikegorelikoff/ai-sdlc-harness/security/policy)

## Maintain and review history

- [Maintain and contribute](../maintainers/index.md)
- [Release history and versions](../reference/versions.md)
- [Documentation decision log](decision-log.md)
- [Audit archive](audits.md)

## AI SDLC product family

AI SDLC Harness structures delivery. [AI SDLC Loop](https://mikegorelikoff.github.io/ai-sdlc-loop/)
provides a smaller approval-gated delivery cycle. Context Guard controls
avoidable context growth and retains local evidence. AI SDLC Metrics measures
local adoption from available evidence. They are complementary, independently
installed products. Harness pins Loop for compatibility and promotion testing;
no built-in technical integration with Context Guard or Metrics is implied.
