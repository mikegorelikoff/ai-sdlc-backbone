...
| Unsupported host or global install requested | Compare the requested host/scope with [supported environments](../reference/supported-environments.md) and preserve the requested business need. | Use the validated project-scoped Codex path; otherwise return a blocker and open a bounded conformance task for the new combination. | Expected: project validator passes and pilot evidence names Codex and `.agents/skills/`; no unsupported-scope claim remains. | Do not redirect the native installer into a home directory or relabel a copied directory as validated support. | Another host or global scope is materially required. |
