...
| --- | --- | --- | --- | --- | --- |
| Install command fails | Run `git --version`, `python3 --version`, and `git status --short`; verify the exact-fetch checkout in the [install guide](../how-to/install.md) resolves to the documented SHA and is clean. | If origin, revision, Codex project scope, and existing managed differences are accepted, retry the native installer from the same checkout; otherwise return a blocker. | Run the installed `ai_sdlc_install_record.py` and flow `--help`; expected inventory, lock, digests, and imports pass. | Do not paste credentials, bypass dirty-source or digest gates, add global scope, or delete unrelated files to force installation. | Origin, installed ownership, scope, network policy, revision, or requested replacement authority differs. |
| Helper import fails after install | Run `python3 .agents/skills/ai-sdlc-flow/scripts/flow.py --help` and inspect the installed shared-runtime inventory. | Reinstall the accepted version with `ai-sdlc-shared-runtime`; if the source version is unknown, stop with a package-owner blocker. | Re-run flow and SDD `--help`; expected usage renders from both with no traceback. | Do not patch an installed runtime copy ad hoc or add source-tree fallback imports. | Source and installed inventory disagree after a clean reinstall. |
...
| Unsupported host or global install requested | Compare the requested host/scope with [supported environments](../reference/supported-environments.md) and preserve the requested business need. | Use a named project profile or `agent-project --skills-root <relative-path>` for an Agent Skills-compatible host; global scope remains blocked. | Run the recorded root's `ai_sdlc_install_record.py`; expected: validation passes and pilot evidence names the profile, skills root, and exact package-versus-host conformance claim. | Do not redirect the installer into a home directory, guess a vendor path, or relabel package placement as model-host certification. | The host uses another package format, lacks a project root, or global scope is materially required. |
...
| Research network/source failure | Preserve the question/source record and use the [web research protocol](../reference/skills/ai-sdlc-research.md) to open the direct locator and check freshness. | Retry only approved internet access or return a sourced blocker; do not substitute cached model knowledge. | Re-run the authoritative research helper on `/tmp/research.toon`; expected direct HTTP(S) sources, access dates, citations, and limitations validate. | Do not cite search-result pages, fabricate access dates, or present model memory as current evidence. | Access rights, source authenticity, legal use, or required current evidence remains unresolved. |
