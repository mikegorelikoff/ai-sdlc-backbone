...
artifact_metadata:
  ...
  path: "specs-refiniment/013-role-guided-installable-flow/discovery.md"
...
- Contributors reported unclear skill order, unreadable recovered context, role overreach, hidden bugs, and a source-only shared directory that could not be installed.

## Current Process and Alternatives
- Current flow hard-codes routing and uses dual shared-helper locations. Alternatives considered were vendoring, a compatibility shim, or one canonical runtime; DEC-001 selects the canonical runtime.

## Value Proposition and Business Goals
- Make the next action, active role, selected evidence, and installation dependency understandable without increasing entrypoint count.

## Users, Roles, and Scenarios
- Primary users are contributors and maintainers. The five canonical operating roles cover product, backlog, analysis, quality, and engineering work.

## MVP and Priorities
- MVP includes runtime consolidation, five role cards, selector/JIT flow V2, bounded configuration, docs generation, and install regression coverage.

## Functional and Non-Functional Needs
- Routing must be deterministic, explainable, safe under untrusted repository evidence, portable after selective installation, and stable under repeated execution.

## Operations, Launch, and Support
- Deliver in the 2.0 release-candidate line with migration diagnostics, project/global install checks, troubleshooting updates, and no automatic external writes.

## Discovery Risks and Dependencies
- Highest risk is import/path breakage across consumers; next is role or action misrouting. Both require exhaustive deterministic tests.
